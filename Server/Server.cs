...
public static class Server
{
    ...
    private static void _Await()
    {
        ...
        while (true)
        {
            try
            {
                ...
                Thread listenerThread = new Thread(receiveMethod) { IsBackground = true };
                //开始监听该客户端发送的消息
                listenerThread.Start(player);
            }
            catch (Exception ex)
            {
    ...
    private static void _Receive(object obj)
    {
        ...
        Socket clientSocket = player.Socket;
        ...
        while (true)
        {
            ...
            try
            {
                receive = clientSocket.Receive(data); //同步接受消息
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{clientSocket.RemoteEndPoint}已掉线:{ex.Message}");
                player.Offline();
                return;
            }
            ...
            {
                Console.WriteLine($"{clientSocket.RemoteEndPoint}已掉线");
                player.Offline();
                return;
            }
